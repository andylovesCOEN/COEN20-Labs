...
MxPlusB:
        ...
        MULS.N R1, R0, R1	    // R1 = dvnd; R2 - dvsr (Assume product does not overflow)
        SMULL R4, R0, R2, R1	// R0 = dvnd*dvsr       (Sign of product = Sign of x*mtop*mbtm)
        ...
        LDR R5, =2	            // R5 = 2
        SDIV R0, R0, R5         // R0 = rounding = (((dvnd*dvsr) >> 31) * dvsr) << 1) + dvsr) / 2
        ADD R0, R0, R1          // R0 = dvnd + rounding
